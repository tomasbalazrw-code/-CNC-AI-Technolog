...
async function uploadFile(base64, fileName, mime) {
  ...
  const r = await fetch(`${OPENAI_URL}/files`, { method: "POST", headers: { Authorization: `Bearer ${OPENAI_API_KEY}` }, body: form });
...
export default async function handler(req, res) {
  try {
    ...
    if (!OPENAI_API_KEY) return fail(res, 500, "OPENAI_API_KEY nie je nastavený vo Verceli.");
    ...
    if (mime === "application/pdf" || /\\.pdf$/i.test(fileName)) {
      const uploaded = await uploadFile(base64, fileName, "application/pdf");
